<!-- Chemin: frontend/src/components/products/StockAdjustModal.vue -->
<template>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="section">
          ...
          <!-- ✅ NOUVEAU : Sélection de l'unité -->
          <div class="form-group">
            <label class="required">Type d'unité</label>
            <select
              v-model="form.unit_type"
              required
              :class="{ error: errors.unit_type }"
            >
              <option value="">Sélectionner une unité</option>
              <option value="base">{{ product.base_unit?.name || 'Casier/Pack' }} (unité de base)</option>
              <option value="retail">{{ product.retail_unit?.name || 'Unité individuelle' }}</option>
            </select>
            <span v-if="errors.unit_type" class="error-message">{{ errors.unit_type }}</span>
            
            <!-- Aide contextuelle -->
            <div v-if="form.unit_type" class="unit-help">
              <span v-if="form.unit_type === 'base'" class="help-text">
                ℹ️ {{ form.quantity }} {{ product.base_unit?.name || 'casier(s)' }} = 
                {{ form.quantity * (product.base_unit_quantity || 1) }} {{ product.retail_unit?.name || 'unité(s)' }}
              </span>
              <span v-if="form.unit_type === 'retail'" class="help-text">
                ℹ️ {{ form.quantity }} {{ product.retail_unit?.name || 'unité(s)' }} individuelle(s)
              </span>
            </div>
          </div>
...
// ✅ Quantité réelle en unités individuelles
const realQuantity = computed(() => {
  const qty = form.value.quantity || 0
  if (form.value.unit_type === 'base') {
    // Convertir les casiers en unités individuelles
    return qty * (props.product.base_unit_quantity || 1)
  }
  // Déjà en unités individuelles
  return qty
})
...
const showOutOfStockError = computed(() => {
  return operationType.value === 'out' && realQuantity.value > props.product.stock
})
...
function validateForm() {
  ...
  if (!form.value.unit_type) {
    errors.value.unit_type = 'Le type d\'unité est requis'
  }
  
  if (operationType.value === 'out' && realQuantity.value > props.product.stock) {
    errors.value.quantity = `Stock insuffisant (${props.product.stock} unités disponibles)`
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    // ✅ Appeler l'API avec unit_type
    const result = await window.electron.apiCall('POST', '/stock/adjust', {
      product_id: props.product.id,
      type: operationType.value,
      quantity: form.value.quantity,
      unit_type: form.value.unit_type, // ✅ NOUVEAU
      reason: finalReason
    })
    
    if (result.success) {
      emit('adjusted', result.data)
      close()
    } else {
      submitError.value = result.message || 'Erreur lors de l\'ajustement du stock'
    }
  } catch (error) {
...
<style scoped>
/* Réutiliser les styles de StockManagementModal.vue */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
...
/* ✅ NOUVEAU : Aide contextuelle pour les unités */
.unit-help {
  margin-top: 8px;
  padding: 8px 12px;
  background: #dbeafe;
