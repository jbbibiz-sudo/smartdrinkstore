<template>
  <aside :class="['bg-gray-800 text-white flex flex-col shadow-lg transition-all duration-300', isCollapsed ? 'w-16' : 'w-64']">
    ...
    <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
      ...
      >
        <span class="text-xl">🍾</span>
        <span class="flex-1 ml-2">Consignes</span>
        <!-- Badge conditionnel si vous avez des conteneurs vides -->
        <span v-if="!isCollapsed && depositsCount > 0" class="badge">
          {{ depositsCount }}
        </span>
      </a>
      ...
      <a 
        ...
        @click.prevent="$emit('navigate', 'users')"
        :class="['nav-item', {'nav-item-active': currentView === 'users'}]"
        href="#"
